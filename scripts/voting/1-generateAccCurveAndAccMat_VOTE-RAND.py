	...
	for time in times:
		...
		for voters in range(1, NUM_VOTERS + 1):
			...
			for GROUP_SIZE in range(5, MAX_GROUP_SIZE + 1, 5):
				...
				np.savetxt('./accuracyMatrices/5-AccMat-RAND_' + str(voters) + '_' + str(GROUP_SIZE) + '_' + time + '.csv', accMat, delimiter=',') 
			...
			np.savetxt('5-accCurveNoSameTake-RAND-' + str(voters) + '_' + time + '.csv', accCurveState, delimiter=',')
