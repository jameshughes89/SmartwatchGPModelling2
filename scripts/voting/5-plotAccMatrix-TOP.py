...
for time in times:
	...
	for voters in range(NUM_VOTERS_START, NUM_VOTERS_STOP + 1):
		fName = '1-AccMat-TOP_' + str(voters) + '_' + str(GROUP_SIZE) + '_' + time + '.csv'
		largeMat = loadData(fName)
		for i in range(len(largeMat)):
			largeMat[i,i] = float('nan')

		fName = '1-AccMat-Small-TOP_' + str(voters) + '_' + str(GROUP_SIZE) + '_' + time + '.csv'
		...
		if time == '':
			...
			name = time	
		plt.suptitle('Confusion Matrix --- Top: ' + name + ', ' + str(voters) + ' voter(s)',fontsize=16)
		plt.show()
