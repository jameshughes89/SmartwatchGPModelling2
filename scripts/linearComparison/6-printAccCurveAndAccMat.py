...
def generateAccCurve(functions, regType = 'OLS'):
	...
	for time in times:
		...
		for GROUP_SIZE in range(5, 121, 5):
			...
			for task in tasks:
				for subject in subjects:
					for take in takes:
						...
						# Do note that this input data is ONE dimensional
						...
						# We'll repeat the test 100 times (to give us a percent)
						...
						for i in range(100):
							...
							# The length of the lists inside this list will be equal to NUM_FUNCTIONS			
							...
							#      np.random.permutation(len(data)) gives us a list of randomly permuted numbers between 0 -- len(data) exclusively
							#      We will then take 0 -- GROUP size of them
                            #      We end up with randomly selected data points
							# I don't remember why I did it this way, but for some reason I think it was faster? Idk
			...
			# Smushing is good because it doesn't matter if we classify the take wrong because we want to find subject/task
			#      In fact, it's impossible to get the take right because we excluded the data the models were fit to
