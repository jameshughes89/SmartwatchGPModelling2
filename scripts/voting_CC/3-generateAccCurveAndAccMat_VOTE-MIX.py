	...
	for time in times:
		...
		for voters in NUM_VOTERS:
			...
			for GROUP_SIZE in range(5, MAX_GROUP_SIZE + 1, 5):
					...
					for subject in subjects:
						for take in takes:
							...
							for i in range(100):
								...
								# Select a constant, random group of voters FROM THE BEST 50% for the group of data
								# WARNING. The use of task, subject, take instead of tsk, sub, and tke here is weird, but safe
								# It's only being used to get a size. 
								ourVoters = np.random.permutation(len(functions[time][task][subject][take])/2)[:voters]
				...
				np.savetxt('./accuracyMatrices/3-AccMat-MIX_' + str(voters) + '_' + str(GROUP_SIZE) + '_' + time + '.csv', accMatSmall, delimiter=',') 
				...
				np.savetxt('./accuracyMatrices/3-AccMat-Small-MIX_' + str(voters) + '_' + str(GROUP_SIZE) + '_' + time + '.csv', accMat, delimiter=',') 
